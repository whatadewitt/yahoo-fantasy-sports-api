...
**Architecture:** A shared transport/XML foundation (`api()` gains PUT/DELETE + XML bodies; new `xmlHelper.ts`) plus feature methods on existing namespaces. Phase A = foundation + roster editing (branch off `master`). Phase B = transaction writes (branch off Phase A's branch; rebased onto `master` after A merges). Unit tests only — mock `https` via `nock`, assert method/URL/body.
...
Worktree creation is handled by `superpowers:using-git-worktrees` at execution time. Target branch: `write-api-foundation-roster`, based on `master`.
...
gh pr create --base master --title "feat: write API foundation + roster lineup editing" --body "Implements docs/superpowers/specs/2026-05-17-write-api-design.md Phase A: api() PUT/DELETE + XML bodies, xmlHelper (escapeXml, buildRosterPayload), roster.update(). Unit tests only."
```
...
> plumbing from Phase A. After PR A merges, rebase Phase B onto `master`; the
> Task A1–A3 plumbing collapses to an empty diff.
...
gh pr create --base master --title "feat: all transaction write operations" --body "Implements docs/superpowers/specs/2026-05-17-write-api-design.md Phase B: add/drop/waiver/trade creates, trade responses, commissioner actions, edit/cancel. Branched off the foundation PR; rebase onto master after PR A merges. Unit tests only."
```
